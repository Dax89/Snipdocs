...
#define vect_resize(T, self, n) self = (Vect(T))_vect_resize(self, n, sizeof(T))
#define vect_reserve(T, self, cap) self = (Vect(T))_vect_ensure_capacity(self, cap, sizeof(T))
...
#define vect_del(self, idx) _vect_remove(self, idx, sizeof(T))

#define vect_ins(T, self, idx, ...) do{ \
    self = (Vect(T))_vect_ins(self, idx, sizeof(T)); \
    self[idx] = __VA_ARGS__; \
} while(0)

#define vect_add(T, self, ...) do{ \
    self = (Vect(T))_vect_ins(self, (uintptr_t)-1, sizeof(T)); \
...
inline Vect(void) _vect_ensure_capacity(Vect(void) self, uintptr_t newcap,
    ...
    VectHeader* newhdr =
        (VectHeader*)calloc(1, sizeof(VectHeader) + (itemsize * newcap));
    memcpy(newhdr, hdr, sizeof(VectHeader) + (itemsize * hdr->length));
...
inline Vect(void)
    _vect_resize(Vect(void) self, uintptr_t newn, uintptr_t itemsize) {
    self = _vect_ensure_capacity(self, newn, itemsize);
    vect_header(self)->length = newn;
    return self;
}

inline Vect(void)
    _vect_ins(Vect(void) self, uintptr_t idx, uintptr_t itemsize) {
    ...
    self = _vect_ensure_capacity(self, newlen, itemsize);

    // If we are appending (pos == len), skip memmove
    if(idx < len) {
        // Shift existing content after the position
        // (+1 to include null terminator)
        memmove((char*)self + ((idx + 1) * itemsize),
                (char*)self + (idx * itemsize), (len - idx) * itemsize);
    }
...
inline void _vect_del(Vect(void) self, uintptr_t idx, uintptr_t itemsize) {
    VectHeader* hdr = vect_header(self);
    ...
    if(idx < len - 1) {
        memmove((char*)self + (idx * itemsize),
                (char*)self + ((idx + 1) * itemsize),
                (len - idx - 1) * itemsize);
    }
...
inline void vect_del_n(Vect(void) self, uintptr_t start, uintptr_t n,
    ...
    memmove((char*)self + (start * itemsize), (char*)self + (end * itemsize),
            (hdr->length - end) * itemsize);
