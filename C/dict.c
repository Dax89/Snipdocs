...
static const void* _dict_getnodekey(const _Dict* self, // NOLINT
                                    const DictNode* n) {
    if(self->ksize == -1U)
        return *(const char* const*)((const uint8_t*)n + sizeof(DictNode*));
    return (const uint8_t*)n + sizeof(DictNode*);
}
...
static void _dict_freenode(_Dict* self, DictNode* n) { // NOLINT
    if(self->nodedestroy) self->nodedestroy(n);
    free(n);
}
...
static bool _dict_insertnode(_Dict* self, DictNode* n) { // NOLINT
    ...
    if(self->buckets[idx].next) {
        ...
        while(*ncurr) {
            ...
            if(self->equal(key, currkey, self->ksize)) {
...
_Dict _dict_create(uint32_t nsize, uint32_t ksize, bool str) {
    return (_Dict){
        .capacity = 0,
        .length = 0,
        .nsize = nsize,
        .ksize = str ? -1U : ksize,
        .buckets = NULL,
        .equal = str ? _dict_defaultequalstr : _dict_defaultequal,
...
const DictNode* _dict_get(_Dict* self, const void* key) { // NOLINT
    if(!self) return NULL;

    size_t idx = self->hash(key, self->ksize) % self->capacity;
    const DictNode* ncurr = self->buckets[idx].next;

    while(ncurr) {
        const void* currkey = _dict_getnodekey(self, ncurr);
        if(self->equal(key, currkey, self->ksize)) return ncurr;
        ncurr = ncurr->next;
    }
